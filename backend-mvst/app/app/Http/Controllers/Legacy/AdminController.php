...
class AdminController extends Controller
{
    ...
    public function liste(Request $request): JsonResponse
    {
        ...
        try {
            $admins = DB::select(
                'SELECT id, telephone, role, gare, nom, prenoms, mail, "dateDeCreation", "peutGererSuggestions", "peutGererLesNotificationsPush", "peutPositionnerCar" FROM "Admins" ORDER BY "dateDeCreation" DESC'
            );
    ...
    public function modifierPermissionPositionnerCar(Request $request): JsonResponse
    {
        if (! $this->resolveur->exigerSuperadmin($request)) {
            return response()->json(['success' => false, 'message' => 'Accès non autorisé'], 200);
        }
        ...
        try {
            ...
            if (! isset($data['id']) || ! isset($data['peutPositionnerCar'])) {
                return response()->json(['success' => false, 'message' => 'Paramètres manquants'], 200);
            }

            $valeur = filter_var($data['peutPositionnerCar'], FILTER_VALIDATE_BOOLEAN);

            $affected = DB::update(
                'UPDATE "Admins" SET "peutPositionnerCar" = :valeur WHERE id = :id',
                ['valeur' => $valeur, 'id' => $data['id']]
            );
